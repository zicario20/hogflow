...
(`Implement Phase 10.2 pig detector integration`). Phase 10.3 se
publica mediante el commit
`Implement Phase 10.3 real-world validation workflow`; su SHA final
...
validado, la cámara física validada, almacenamiento y evaluación
contra ground truth. Phase 9.2 ofrece un workflow de operador ejecutable,
in-memory y protegido por snapshots/confirmaciones, sin validar usabilidad
operacional. Phase 9.3 añade una fuente camera/file y un worker compartidos con
routing lifecycle-safe. Phase 9.4 añade un preview local de una sola ranura,
overlays en el hilo UI, diagnósticos y recuperación USB acotada; su evidencia
sigue siendo sintética y no contiene detecciones o conteos reales de cerdos.
...
Phase 10.3 añade autorización estricta de tres videos locales, gate de modelo,
modelos de evidencia path-free, calibración offline por video y reportes
sanitizados. Los tres videos fueron legibles en inspección metadata-only, pero
no existían sidecars, modelo compatible ni ground truth manual; por ello no se
ejecutó detector/tracker/crossing/counting y el veredicto empírico fue `REAL
DETECTOR VALIDATION COULD NOT BE COMPLETED`.
...
- `counting` no importa `sessions`; los coordinadores Phase 8.2–8.4 mantienen la
  dirección sin ciclo.
- `camera` Phase 9.3–9.4 consume contratos públicos framework-neutral de
  source, detector, tracker, crossing y runtime; publica como máximo un frame
  visual reemplazable y no importa presentation/Tkinter.
- `application` Phase 9.1–9.4 consume APIs públicas de `domain`, `sessions` y
...
- `presentation` construye overlays y objetos Tk únicamente en el hilo UI; un
  error visual se registra de forma acotada y no cambia el pipeline/count.
- `bootstrap` y `__main__` son la capa superior de composición Phase 9.4; ningún
...
| `OperatorPresenter`, `OperatorView`, `OperatorScreen` | `src/hogflow/presentation/` | Boundary sin business state ni frameworks CV; muestra acciones, confirmaciones, health y solicita el último frame mediante application. |
| `PreviewRenderPlan` | `src/hogflow/presentation/preview.py` | Plan puro de primitivas de overlay; Tkinter materializa PPM/canvas en el hilo UI. |
| `OperatorRuntimeComposition`, `OperatorDesktopComposition` | `src/hogflow/bootstrap.py` | Composition root Phase 9.4; crea el único runtime/source/pipeline/canal visual y enlaza presenter/view una sola vez. |
| `ProductionRuntimeConfiguration`, `RuntimeHeartbeat`, `RuntimeDiagnosticsSnapshot` | `src/hogflow/runtime/models.py` | Configuración y proyecciones Phase 10.1 inmutables, sanitizadas y de memoria acotada. |
| `RuntimeHealthManager` | `src/hogflow/runtime/health.py` | Agrega health/FPS/latency/fallos/restarts bajo cadence del caller, sin retener heartbeats. |
| `ProductionRuntimeSupervisor` | `src/hogflow/runtime/supervisor.py` | Boundary explícito para heartbeat y restart del único pipeline/preview con guard de lane activa. |
...
| `docs/phase_2/` | Foundation, contratos, integración, reglas y ADR-001–061. |
...
| `docs/phase_10/` | Runtime Phase 10.1, detector local Phase 10.2 y workflow/gates Phase 10.3, con evidencia y limitaciones explícitas. |
...
| `src/hogflow/camera/` | Orquestación Phase 9.3–9.4 de una fuente/worker, processor detector→tracker→crossing, canal visual latest-only y recuperación USB acotada. |
...
| `src/hogflow/validation/` | Phase 10.3: catálogo autorizado, gates locales, modelos path-free, calibración, workflow serial y reportes sanitizados. |
...
- `python -m hogflow.video.real_world_validation_cli`: metadata/gate/reporte local Phase 10.3 sin UI ni retención de media;
...
- `python -m hogflow` o `hogflow run`: Operator MVP Phase 9.4 con composición
  local camera/file, preview diagnóstico latest-only opcional (`--disable-preview`)
  y runtime in-memory.

Existe la workstation ejecutable Phase 9.4 con preview local, overlays,
diagnósticos y recuperación USB acotada, pero no dashboard analítico,
validación física/pig-specific ni base de datos. Phase
...
| Phase 9.4 | Preview local latest-only, overlays diagnósticos, métricas y recuperación USB acotada integrados sin afectar conteo. | Frames RGB, fallos, recovery y UI scheduling sintéticos/headless; sin cámara/display ni cerdos reales. | Implementación técnica completa según alcance; usabilidad, hardware y evidencia pig-specific pendientes. |
| Phase 10.1 | Heartbeat, health, memoria, diagnostics y restart controlado sobre el runtime Phase 9.4. | 10.000 heartbeats, fallos/restarts y boundaries sintéticos; sin soak real ni hardware. | Foundation técnica completa según alcance local; producción no validada. |
...
### 5.23 Phase 9.4 — Live Operator Experience, Video Preview and Diagnostics

- **Objetivo:** completar Phase 9 con un preview local no autoritativo,
  diagnósticos live y recuperación de cámara acotada sin ralentizar counting.
- **Entregado:** `LatestPreviewFrameChannel` de una ranura, modelos visuales
  inmutables, publicación RGB desde el processor, overlays puros, Canvas/PPM
  materializado por Tk, un único callback UI acotado, FPS/health/recovery y
  APIs application-only para disponibilidad, frame y métricas.
- **Decisiones:** ADR-058; no existe cola ni historia visual; el worker nunca
  llama Tkinter; preview consume después de tracking/crossing exitoso y sus
  fallos no mutan pipeline/counting; USB reintenta con umbral, límite y delay
  explícitos, mientras file/EOF no reconecta.
- **Evidencia:** tests sintéticos/headless de reemplazo, startup/close,
  thread-safety, overlays, render failure isolation, FPS, pérdida/reapertura,
  lifecycle y boundaries. No se abrió cámara física ni display.
- **Commit:** `Implement Phase 9.4 live operator experience and diagnostics`
  (consultar SHA final en Git por la autorreferencia del documento).
...
- **Objetivo:** preparar el runtime Phase 9.4 para observación y recuperación
  controlada de larga duración sin cambiar reglas, UI, detector ni threading.
- **Entregado:** paquete `hogflow.runtime` con configuración fingerprinted,
  heartbeat/health/diagnostics inmutables, probe stdlib de memoria residente,
  warnings con `deque` fijo y supervisor síncrono de restart. El snapshot del
  pipeline añade frame procesado exacto, fallos por etapa, drops y latencia.
- **Decisiones:** ADR-059; cadence pertenece al caller; no monitor thread ni
  queue; camera/pipeline restart recrea el único worker y se bloquea con lane
  ocupada; preview restart no toca counting.
- **Evidencia:** suites sintéticas de modelos, health, failure classification,
  restart, determinismo, arquitectura y 10.000 heartbeats sin historia.
- **Commit:** `Implement Phase 10.1 production runtime foundation` (consultar
  SHA final en Git por autorreferencia).
...
### 5.26 Phase 10.3 — Real-World Detector, Tracking and Counting Validation

- **Objetivo:** ejecutar evidencia controlada y separada sobre tres videos
  locales exactos, con hard gate de modelo y ground truth, sin retener media ni
  fabricar métricas.
- **Entregado:** paquete `hogflow.validation`; catálogo exacto y ordenado;
  inspección de ignore/tracked/sidecar/model; estados `MEASURED`, `PROVIDED
  MANUAL GROUND TRUTH`, `DERIVED`, `UNKNOWN` y `NOT APPLICABLE`; candidatos y
  planes de calibración que reutilizan Phase 6; backend model-present inyectable;
  reportes JSON/Markdown deterministas; CLI headless y tests sintéticos.
- **Decisiones:** ADR-061; Video 1 primero, Video 2 solo tras éxito estructural,
  Video 3 solo stress; un modelo distinto de `AVAILABLE` impide invocar backend;
  manual total no produce precision/recall/F1; outputs solo en roots ignorados.
- **Evidencia local:** los tres videos exactos existían, estaban ignorados y
  eran legibles. Metadata: `video_1` 971 frames, 848×384, 60.0185 FPS nominal;
  `video_2` 333 frames, 1248×576, 27.3324 FPS nominal; `video_3` 575 frames,
  848×384, 59.8854 FPS nominal. No había sidecars, modelo compatible ni ground
  truth. No se ejecutó inferencia.
- **Veredicto:** `REAL DETECTOR VALIDATION COULD NOT BE COMPLETED`.
- **Commit:** `Implement Phase 10.3 real-world validation workflow` (consultar
  SHA final en Git por autorreferencia).
...
| 10 | Runtime, detector, validación controlada y SQLite para sesiones/eventos. | 10.1–10.3 IMPLEMENTED según infraestructura; real validation BLOCKED; persistence/10.4 NOT STARTED |
...
| Actualización de Tkinter desde el worker. | Rechazada en Phase 9.3 y preservada en Phase 9.4 | Tkinter no es thread-safe y el worker no debe poseer presentación. | No reconsiderar; Phase 9.4 usa un único callback de UI aislado, acotado y cancelable. |
| Cola o historial de frames de preview. | Rechazada en Phase 9.4 | Acumularía memoria/latencia y podría ejercer backpressure sobre counting. | No reconsiderar para monitor live; playback/recording requeriría otra fase y data-governance explícita. |
| Reconnect automático infinito. | Rechazada en Phase 9.4 | Ocultaría una fuente fallida, dificultaría shutdown y mantendría loops sin límite. | Solo ampliar límites mediante configuración y evidencia de backend autorizada; nunca loop ilimitado. |
...
| Línea base técnica de Phase 9.4 | `a2088d02860f42cb21a018f43ee3bffab152c2e4` |
| `origin/main` al iniciar Phase 9.4 | Mismo SHA que la línea base |
...
| Línea base técnica de Phase 10.3 | `e14bf5b5d73b886ff9834b606787ca58872c65b2` |
| `origin/main` al iniciar Phase 10.3 | Mismo SHA que la línea base |
| Working tree al iniciar Phase 10.3 | Limpio; `.agents/`, media local y outputs de evaluación ignorados |
...
| Suite local Phase 9.4 | 868 passed; 1 warning heredado de ByteTrack deprecated |
...
| Suite baseline Phase 10.3 | 941 passed; 1 warning heredado de ByteTrack deprecated |
| Suite enfocada Phase 10.3 | 64 passed para modelos, workspace, workflow, reporting y arquitectura |
...
- integración secuencial session/counting y coordinación síncrona de cuatro
  dock records con un único carril/counter/source, snapshots y shutdown explícito;
- workflow Phase 9.1–9.4 de operador sobre comandos públicos, presenter sin
...
- preview local latest-only con overlays diagnósticos, FPS, health/recovery,
  publicación visual aislada y render Tk exclusivamente en el hilo UI;
- heartbeat caller-driven con health de worker/camera/pipeline/preview/counter/
  lane, memoria residente, FPS/latency/failure/restart aggregates y warnings
  acotados, sin nuevo hilo ni historial;
- reinicio explícito de camera/pipeline reutilizando el único worker con guard
  de lane activa, y reinicio independiente del preview;
...
- adapter desktop Tkinter lazy, manual refresh de negocio y un único redraw
  visual acotado sin display requerido en CI;
...
- cámara física Phase 9.4, detector pig-specific y evidencia de conteo real;
...
- SQLite, broader operator review workflow persistente, dashboard, analytics y
  review clips;
...
| HF-D039 | Media de UX/performance | **HECHO VERIFICADO SOLO EN TESTS HEADLESS** | `TkOperatorView`; `PreviewRenderPlan` | El canal, scheduler y overlays son deterministas, pero no se midieron fluidez, CPU, memoria, legibilidad o accesibilidad en un display/cámara reales. | Ejecutar smoke autorizado de workstation y perfilar render/FPS sin alterar el counting path. | Abierta empírica; preview técnico implementado. |
| HF-D040 | Alta para count accuracy tras recovery | **INFERENCIA respaldada por diseño** | `CountingPipelineController._try_recover_source`; ADR-058 | La recuperación USB resetea tracker/crossing pero conserva el lifecycle de sesión Phase 7. IDs fragmentados pueden volver a contar; IDs reutilizados pueden causar undercount. | Medir reconnect con ground truth y definir una política operacional antes de aceptar recovery como continuidad exacta. | Recovery técnico acotado implementado; exactitud no validada. |
| HF-D041 | Alta para disponibilidad | **DECISIÓN DE SEGURIDAD** | `ProductionRuntimeSupervisor`; ADR-059 | Reiniciar source/pipeline durante un lifecycle activo resetea identidad temporal y puede corromper count continuity. | Bloquear por default mientras lane está ocupada; definir recovery continuity-safe solo con evidencia y regla autorizada. | Guard implementado; failover activo pendiente. |
...
| HF-D048 | Alta para evidencia Phase 10.3 | **LIMITACIÓN DE EVIDENCIA** | gate/model report Phase 10.3 | Los tres videos autorizados son legibles, pero no hay modelo compatible ni manual/frame ground truth; todo detector/tracking/crossing/counting real permanece sin ejecutar. | Proveer artefacto pig-specific autorizado, ignorado y compatible; declarar candidatos por video y ground truth independiente antes de ejecutar. | Infraestructura completa; evidencia empírica bloqueada. |
| HF-D049 | Alta para count accuracy | **DECISIÓN DE SEGURIDAD** | `video_3`; ADR-061 | Video 3 contiene movimiento multidireccional y no está autorizado como counting candidate. | Mantener count/error `NOT_APPLICABLE`; usar solo para stress detector/tracker. | Enforced por modelo y tests. |
...
- SQLite/storage o Phase 10.4;
- ampliar Phase 9/10.1/10.2/10.3 sin prompt explícito;
...
- auditoría técnica y CI de Phase 10.3 verificados;
...
- **CONFIRMADO / implementado:** Phase 0, Phase 1, Phase 2.1–2.3, tooling Phase
  ...
  8.1–8.4, Phase 9.1–9.4 y Phase 10.1–10.3 según sus alcances.
- **CONFIRMADO / no iniciado:** persistencia Phase 10, Phase 10.4 y Phase
...
15. **COMPLETADO CON BLOQUEO EMPÍRICO:** workflow y reportes Phase 10.3; no
   hubo modelo compatible ni ground truth, por lo que no se ejecutó inferencia.
16. **PENDIENTE:** auditar Phase 10.3 y aportar prerequisites autorizados para
   evaluación representativa Phase 6/7/8.
17. **PENDIENTE:** triage de deuda del tracker y plan de migración Supervision.
18. **PENDIENTE:** asegurar backup privado/reproducible de datos locales.
...
- Phase 9.4: preview latest-only, overlays, health y recovery acotado implementados;
- Phase 9: técnicamente completada según alcance; validación operacional pendiente;
...
| `CountingPipelineController` | Orquestador Phase 9.3–9.4 de una fuente y worker compartidos; produce snapshots sanitizados, publica preview y enruta crossing evidence sin poseer el counter. |
...
| `PreviewFrame` | Frame visual RGB24 inmutable con overlays diagnósticos; no contiene ni sustituye snapshots de negocio. |
| `LatestPreviewFrameChannel` | Ranura thread-safe que reemplaza el frame anterior y nunca acumula historia. |
| `PreviewSnapshot` | Estado/métricas acotadas del canal visual, sin pixels ni objetos GUI. |
| `CameraRecoveryConfiguration` | Umbral, máximo de reintentos y delay explícitos para recuperación de fuente USB; no aplica a EOF de file. |
| `OperatorApplicationService` | Boundary Phase 9.1–9.4 que traduce acciones del operador a métodos públicos del coordinator/pipeline, coordina shutdown y expone snapshots/frame visual. |
...
| `EvidenceValue` | Valor escalar Phase 10.3 con provenance explícita: medido, manual proporcionado, derivado, unknown o not applicable. |
| `CalibrationCandidate` | Configuración offline fingerprinted y scoped a un video que combina thresholds detector/tracker con un `LineCandidate` Phase 6; no cambia defaults live. |
| `VideoValidationResult` | Resultado path-free, agregado y separado para un video autorizado; no contiene frames, paths, tensores ni historia ilimitada. |
| `ModelPresentValidationBackend` | Puerto inyectable Phase 10.3 para ejecutar un candidato usando contratos públicos existentes cuando un modelo local supera el gate. |
